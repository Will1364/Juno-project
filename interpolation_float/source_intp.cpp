...
Mat interpolateNNAng(Mat img, int arr_size);
...
Mat interpolateNN(Mat img, int arr_size, bool lensCorrection) {
	...
	Mat interpolated(height_i, width_i, CV_32FC1);
	interpolated = 0;
	...
	for (int i = 0; i < height_i; i++) {
		for (int j = 0; j < width_i; j++) {
			...
			if (min_d[0] < min_dist) {
				interpolated.at<float>(i, j) = img.at<Vec3d>(min_i[0], min_j[0])[2];
				count_mindist++;
			}
...
Mat interpolateNNAng(Mat img, int arr_size) {
	// Use Mat from imShiftCoordAng

	// Variables : 
	int search = 50;		// NxN search area around each pixel. 50 is a good balance between compute time and extra buffer 
	int padding = 0;		// Number of lines in original image to not include
	float max_dist = 9999;	// The furthest away 'closest' pixel can be 
	float min_dist = 0.00;  // If min distance is closer than this, uses uninterpolated value. Set to zero or negative to disable
	...
	Mat coordGrid(height_i, width_i, CV_64FC2);
	...
	for (int i = 0; i < height_i; i++) {
		for (int j = 0; j < width_i; j++) {
			...
			coordGrid.at<Vec2d>(i, j)[0] = atan2(vec.at<double>(0, 1), vec.at<double>(0, 0));
			coordGrid.at<Vec2d>(i, j)[1] = acos(-vec.at<double>(0, 2));
	...
	for (int i = 0; i < height_i; i++) {
		for (int j = 0; j < width_i; j++) {
			...
			theta_intp = coordGrid.at<Vec2d>(i, j)[0];
			rho_intp = coordGrid.at<Vec2d>(i, j)[1];
			...
			for (int k = 0; k < search; k++) {
				for (int l = 0; l < search; l++) {
					...
					if (s_y + k > padding and s_y + k < height - padding and s_x + l > padding and s_x + l < width - padding) {
						...
						// Calculate distance
						theta_org = img.at<Vec3d>(s_y + k, s_x + l)[0];
						rho_org = img.at<Vec3d>(s_y + k, s_x + l)[1];

						//dist = acos(sin(theta_org)* sin(theta_intp) + cos(theta_org)*cos(theta_intp)*cos(abs(rho_org-rho_intp)));
						dist = acos(sin(rho_org) * sin(rho_intp) + cos(rho_org) * cos(rho_intp) * cos(abs(theta_org - theta_intp)));
...
double intpCoreDist(vector<float> min_d, vector<float> min_v) {
	...
	float p = 1.2;			// Power for distance weight. Closer to one -> distance matters less 
