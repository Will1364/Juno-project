...
pair<Mat, Mat> splitBinary(Mat img, float threshold) {
...
pair<Mat, Mat> splitFloat(Mat img) {
	// Split image into two binaries
	// For use with coordinateLSQ function
	// 
	// Input: Intervowen image
	...
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
			...
			// Even rows
			if (i % 2 + border_pad % 2 == 0) {
				img_2.at<float>(i, j) = 0;
			}

			// Odd rows
			else {
				img_1.at<float>(i, j) = 0;
			}
...
Mat imShiftSimple(Mat img, array<float, 3> rot_prm) {
	...
	bool rotate_first_frame = true;


	float angle = 0; 
	// Parameter setup
	if (rotate_first_frame) {
		angle = -rot_prm[2];
	}
	else {
		angle = rot_prm[2];
	...
	Mat img_shift(height, width, CV_32F);
	img_shift = 0;
		...
		for (int j = 0; j < width; j++) {
			...
			if (i % 2 + border_pad % 2 == 0) {
				...
				if (rotate_first_frame) {
					...
					img_shift.at<float>(i, j) = img.at<float>(i, j);
					...
					if (x_shift >= 0 and x_shift < width and y_shift >= 0 and y_shift < height) {
						...
						img_shift.at<float>(i, j) = img.at<float>(y_shift, x_shift);
						vals_used.at<uchar>(y_shift, x_shift) = 0;

					}
					else {
						img_shift.at<float>(i, j) = 0;
						count++;
					}
				}
	...
	cout << "complete" << endl;
	cout << "Fraction of unused values: " << float(count) / (width * height) << endl << endl;
...
pair<Mat, Mat> blobCoordinates(Mat img, float threshold) {
	...
	// Variables 
	int area_min = 50;		// Minimum area for object to count
	int area_max = 600;		// Maximum area
	...
	// Paramter setup
	Mat img_bn = img > threshold;	// Binary image to do blob analysis in

	int width = img.cols;		// Parameter setup
	int height = img.rows;

	// Remove speckles by erosion
	img_bn = erodeLR(img_bn);
	img_bn = erodeLR(img_bn);
	img_bn = erodeLR(img_bn);
	...
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
			if (j < 30) {
				...
				img_bn.at<uchar>(i, j) = 0;
			}

		}
	}

	// dilate back to make stars uniform + connected
	img_bn = dilateBox(img_bn);
	img_bn = dilateBox(img_bn);
	img_bn = dilateBox(img_bn);
	...
	int val = 0;		// Value in binary image

	float star_val = 0;			// For finding maxval in star
	float star_val_peak = 0;
	int star_peak_x = 0;
	int star_peak_y = 0;
			...
			if (img_bn.at<uchar>(i, j) == 255) {
				...
				for (int k = y_min; k < y_max + 1; k++) {
					for (int l = x_min; l < x_max + 1; l++) {
						...
						val = img_bn.at<uchar>(k, l);
						star_val = img.at<float>(k, l);
						...
						if (val == 255) {
							...
							img_bn.at<uchar>(k, l) = 0;
						}

						if (star_val > star_val_peak) {
							star_val_peak = star_val;
							star_peak_x = l;
							star_peak_y = k;
						}
...
void blobFlux(Mat img, float threshold, float noise_floor) {
	// Finds blobs (stars) and determines their center coordinate and area. Returns list of coordinates (x, y). 
	// Does simple errosion/dilation to remove noise/combine stripes to full stars, 
	// and 4-connectedness to determine position/area of stars. 
	// Used in coordinateLSQ.
	...
	// Variables
	int min_flux = 300/255.0;
	int min_area = 100;

	// Paramter setup
	Mat img_bn = img > threshold;	// Binary image for blob analysis
	...
	*/
			...
			if (img_bn.at<uchar>(i, j) == 255) {
					...
					for (int l = x_min; l < x_max + 1; l++) {
						...
						if (img_bn.at<uchar>(k, l) == 255) {
							...
							if (val > 1 * noise_floor) {
								...
								flux += img.at<float>(k, l) - noise_floor;
				...
				for (int k = y_min; k < y_max + 1; k++) {
					for (int l = x_min; l < x_max + 1; l++) {
						...
						img_bn.at<uchar>(k, l) = 0;		// Remove blob from binary image
	...
	imwrite("_imgFluxUsed.png", img_used);
...
array<float, 3> coordinateLSQ(Mat img, bool lensCorrection) {
	...
	// Parameter setup
	pair<Mat, Mat> img_binary = splitFloat(img);
